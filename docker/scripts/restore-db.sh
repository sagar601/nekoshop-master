...
if [ -z "$backup_file" ]; then
  ...
  echo "$(date): could not restore db: missing backup file" >> ${log_file}
  exit 1
fi

if [ -z "$AWS_ACCESS_KEY_ID" ] || [ -z "$AWS_SECRET_ACCESS_KEY" ] || [ -z "$AWS_DEFAULT_REGION" ] || [ -z "$S3_BACKUPS_BUCKET" ]; then
  echo "$(date): could not restore db: missing environment variables" >> ${log_file}
  exit 1
fi

echo "$(date): starting db restoration" >> ${log_file}

aws s3 cp s3://${S3_BACKUPS_BUCKET}/${backup_file} ${backup_file}
gzip -dc ${backup_file} | psql -U postgres

echo "$(date): db restoration finished" >> ${log_file}